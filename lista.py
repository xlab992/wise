...
def merger_playlist():
    ...
    def download_playlist(path_or_url, append_params=False, exclude_group_title=None):
        ...
        # Rimuovi la riga iniziale #EXTM3U
        playlist = '\n'.join(line for line in playlist.split('\n') if not line.startswith('#EXTM3U'))
    
        # Aggiungi parametri a ogni link .m3u8 (se richiesto)
        if append_params:
            playlist_lines = playlist.splitlines()
            for i in range(len(playlist_lines)):
                if '.m3u8' in playlist_lines[i]:
                    playlist_lines[i] += "&h_user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/133.0.0.0 Safari/537.36&h_referer=https://ilovetoplay.xyz/&h_origin=https://ilovetoplay.xyz"
            playlist = '\n'.join(playlist_lines)
    
        # Escludi canali con un determinato group-title (se richiesto)
        if exclude_group_title:
            playlist = '\n'.join(line for line in playlist.split('\n') if exclude_group_title not in line)
    
        return playlist
    
    # Percorsi ai file locali e all'unico URL remoto
    path1 = "channels_italy.m3u8"   # File locale
    path2 = "eventi.m3u8"           # File locale
    url3  = "https://raw.githubusercontent.com/Brenders/Pluto-TV-Italia-M3U/main/PlutoItaly.m3u"  # Remoto
    path4 = "world.m3u8"            # File locale
    
    # Ottieni la directory dello script
    script_directory = os.path.dirname(os.path.abspath(__file__))
    
    # Scarica/carica tutte le playlist
    playlist1 = download_playlist(path1)
    playlist2 = download_playlist(path2, append_params=True)
    playlist3 = download_playlist(url3)
    playlist4 = download_playlist(path4, exclude_group_title="Italy")
    
    # Unisci tutto in un'unica playlist
    lista = playlist1 + "\n" + playlist2 + "\n" + playlist3 + "\n" + playlist4
    
    # Aggiungi intestazione #EXTM3U
    lista = "#EXTM3U\n" + lista
                ...
                if valid_events:
                    ...
                    for event in valid_events:
                        ...
                        for channel in event["channels"]:
                            ...
                            m3u8_content += f"{channel['stream_url']}&h_user-agent=Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/133.0.0.0 Safari/537.36&h_referer=https://ilovetoplay.xyz/&h_origin=https://ilovetoplay.xyz\n"
